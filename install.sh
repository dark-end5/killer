...
main() {
  ...
  echo -e "${CYAN}[INFO] Installing system dependencies for Killer Bot...${NC}"
  ...
  if [ "$PKG" = "pkg" ]; then
    echo -e "${GREEN}[SUCCESS] All dependencies have been installed successfully!${NC} Run \"yarn start\" to launch Killer Bot again."
    echo -e "${PURPLE}[START] Starting Killer Bot with yarn...${NC}"
    yarn start
  else
    echo -e "${GREEN}[SUCCESS] All dependencies have been installed successfully!${NC} Run \"npm start\" to launch Killer Bot again."
    echo -e "${PURPLE}[START] Starting Killer Bot with npm...${NC}"
    npm start
  fi
}
